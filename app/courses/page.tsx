...
const SyllabusModal = ({ course, onClose }: { course: Course, onClose: MouseEventHandler }) => {
  ...
  return (
    ...
    >
      {/* Modal Content */}
      <div
        className="relative w-full max-w-4xl max-h-[90vh] overflow-hidden bg-white rounded-lg shadow-xl p-6 lg:p-10 flex flex-col"
      ...
      >
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-gray-200 pb-4">
          ...
          <button
            onClick={onClose}
            className="absolute top-0 right-0 text-3xl font-light text-gray-500 hover:text-gray-800"
          >
            &times;
          </button>

          {/* Header Title */}
          <h2 className="text-midnightblue text-3xl lg:text-4xl font-semibold mb-2">
            {course.title}
          </h2>
          <p className="text-gunmetalgray text-lg mb-2">{course.goal}</p>
        </div>
            ...
            {course.modules.map((module, index) => (
              <div key={index} className="bg-white border border-gray-200 rounded-lg p-5">
                <h4 className="text-xl font-semibold text-midnightblue mb-3">
                  {module.title}
                  {module.duration && (
                    <span className="text-sm font-normal text-gunmetalgray ml-2">
                      ({module.duration})
                    </span>
                  )}
                </h4>
